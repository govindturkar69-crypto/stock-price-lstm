...
def _loader(X, y, batch_size, shuffle):
    ds = TensorDataset(torch.as_tensor(np.asarray(X, dtype='float32')),
...
def train_classifier(prep, cfg: dict[str, Any], out_dir: str):
    ...
    opt = torch.optim.Adam(model.parameters(), lr=m.get('learning_rate', 0.001),
                           weight_decay=float(m.get('weight_decay', 0.0) or 0.0))
    sched = torch.optim.lr_scheduler.ReduceLROnPlateau(
        opt, mode='min', factor=0.5, patience=t['reduce_lr_patience'], min_lr=1e-06)
